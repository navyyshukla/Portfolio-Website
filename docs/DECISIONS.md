...
## No vector database
The corpus is ~4,000 tokens — an order of magnitude below where retrieval earns
its complexity. Stuffing it whole makes retrieval failure structurally
impossible and removes an embedding service, a vector store, chunk tuning and a
re-index step. Revisit above ~50K tokens (i.e. if a blog is added).
